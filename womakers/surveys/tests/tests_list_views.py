...
class QuestionListViewTestCase(TestCase):
    ...
    def test_questionView_url_accessible_by_path(self):
        response = self.client.get('/')
        self.assertEqual(response.status_code, 200)

    def test_questionView_url_accessible_by_name(self):
        response = self.client.get(reverse('question_list'))
        self.assertEqual(response.status_code, 200)

    def test_questionView_uses_correct_template(self):
        response = self.client.get(reverse('question_list'))
        self.assertTemplateUsed(response, 'surveys/question_list.html')

    def test_questionView_only_enabled_questions(self):
        response = self.client.get(reverse('question_list'))
    ...
    def test_questionView_returning_message_when_no_questions(self):
